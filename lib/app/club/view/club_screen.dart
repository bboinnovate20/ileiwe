...
import 'package:ileiwe/app/club/club_controller.dart';
...
class ClubScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ClubScreen> createState() => _BooksScreenState();
}

class _BooksScreenState extends ConsumerState<ClubScreen> {
  ...
  void initState() {
    super.initState();
    clubController = ClubController(ref: ref);
        ...
        child: Column(
          ...
          children: [
            ...
            FutureBuilder<List<Club>>(
              future: clubController.getClubs(), 
              builder: (context, clubs) {
                final allClubs = clubs.data;

                if(clubs.hasData) {
                  return  Padding(
                      padding: const EdgeInsets.symmetric(horizontal:  20.0, vertical: 10),
                      child: Column(
                        children: allClubs!.map((club) => clubCard(context, club)).toList(),
                      ),
                    );
                }
                if(clubs.hasError) {
  ...
  Container clubCard(BuildContext context, Club club) {
    return Container(
                    ...
                    children: [
                      SizedBox.square(
                        ...
                        child: ClipRRect(
                          ...
                          child: Image.network(club.imagePath, fit: BoxFit.cover, )),
                      ),
                       Expanded(child: Padding(
                        padding: const EdgeInsets.symmetric(horizontal: 10.0),
                        child: Text(club.name, style: const TextStyle(fontWeight: FontWeight.bold),),
                      )),
                      ButtonTwo(label: "Join Now", 
                      action: () => context.push(RoutesName.clubInfo, extra: club),
                      paddingGeo: const EdgeInsets.symmetric(horizontal: 20, vertical: 10),
